...
const CryptoDetails = () => {
 {/* to know the currency we are currently taking a look at we need to get the cinId which will come from the useParams package. useParams takes the id in the url and allows us to use it as a variable */}
  ...
  const [timePeriod, setTimePeriod] = useState('24h'); //time period for our chart for each indiviual crypto
  ...
  return (
    <Col className="coin-detail-container">
      ...
      <LineChart coinHistory={coinHistory} currentPrice={millify(cryptoDetails?.price)} coinName={cryptoDetails?.name} /> {/*passing these data we fetced from API as props so we can use them in the LinChart.jsx */}
      ...
      <Col className="stats-container">
        ...
        <Col className="other-stats-info">
          ...
          {genericStats.map(({ icon, title, value }, index) => (
            <Col className="coin-stats" key={`genericStat-${index}`}>
              <Col className="coin-stats-name">
                <Text>{icon}</Text>
                <Text>{title}</Text>
              </Col>
